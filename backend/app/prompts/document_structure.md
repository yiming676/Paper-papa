...
- 必须遵守用户消息中的“PDF 阅读策略”。如果策略说明已经发送完整 PDF 抽取文本，不要在 `warnings` 中写“本文基于文本摘要”“未完整阅读全文”等错误提示。
...
- `formula_breakdowns` 核心优先，不要求穷尽每个排版公式，但必须覆盖读懂论文主线最重要的公式。`symbols[].symbol` 必须写成可被 KaTeX 渲染的 LaTeX 符号，例如 `x_i`、`\mu`、`\mathcal{L}_{rec}`，不要写成自然语言。
